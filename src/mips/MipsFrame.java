...
public class MipsFrame extends Frame {
    ...
    public tree.Exp externalCall(String s, List<tree.Exp> args) {
        ...
        tree.ExpList expList = utils.Conversor.ListToExpList(args);
...
//	    s.next().accept(cg);
    ...
    private static assem.Instr OPER(String a, Temp[] d, Temp[] s) {
        return new assem.OPER(a, utils.Conversor.ArrayToTempList(d), utils.Conversor.ArrayToTempList(s), null);
    }
        ...
        if (spills != null) {
            if (!spilling) {
                ...
                for (ListIterator<assem.Instr> i = insns.listIterator(); i.hasNext(); ) {
                    ...
                    Temp[] use = utils.Conversor.TempListToArray(insn.use());
                    ...
                    if (use != null)
                        for (int u = 0; u < use.length; u++) {
                            if (use[u] == spills[s]) {
                                ...
                                // TODO: VER COMO RESOLVER ESSA DEPENDENCIA
                                //insn.replaceUse(spills[s], t);
                                break;
                            }
                        }
                    Temp[] def = utils.Conversor.TempListToArray(insn.def());
